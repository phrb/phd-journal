...
***** GPR Code
#+begin_SRC R :results output :session *R* :eval no-export :exports results :tangle "gpr.r"
library(DiceKriging)
library(dplyr)
...
#+end_example
***** Parsing Data
#+begin_SRC R :results graphics output :session *R2* :file "comparison_histogram.pdf" :width 7 :height 8 :eval no-export
library(ggplot2)
library(dplyr)
...
gpr_lin_data <- read.csv("gpr_lin_best_points.csv") %>%
    select(slowdown, method, measurement_order, time_per_pixel) %>%
    mutate(method = "GPRL")
