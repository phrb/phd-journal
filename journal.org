...
ggplot(data) +
    geom_histogram(aes(cost_mean), binwidth = 0.01) +
    geom_vline(aes(xintercept = cost_mean), O3_baseline, color = "black", linetype = 2) +
    coord_cartesian(xlim = c(0, max(data$cost_mean)), ylim = c(0, 170)) +
    geom_text(aes(x = cost_mean - 0.02, y = 170, label = "-O3 baseline"), data = O3_baseline) +
    theme_bw()
#+END_SRC

#+RESULTS:
[[file:/tmp/babel-ppprzS/figurezCWUzD.png]]
