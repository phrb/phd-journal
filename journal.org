...
#+RESULTS:
[[file:/tmp/babel-Zspy18/figureMuK9H7.png]]
*** [2018-07-19 Thu]
...
#+RESULTS:
[[file:/tmp/babel-Zspy18/figurePjtyj6.png]]
...
#+BEGIN_SRC R
library(ggplot2)
data <- read.csv("orio_experiments/testsuite/SPAPT/stencil3d/xeon_e5_2630/stencil_O3_nobinary/search_space.csv")
...
#+RESULTS:
[[file:/tmp/babel-Zspy18/figureFGWnK1.png]]
...
#+BEGIN_SRC R
library(ggplot2)
data <- read.csv("orio_experiments/testsuite/SPAPT/stencil3d/xeon_e5_2630/stencil_O3/search_space.csv")
...
#+RESULTS:
[[file:/tmp/babel-C2GnlH/figureI5LCRN.png]]
*** [2018-07-24 Tue]
...
'data.frame':	1 obs. of  57 variables:
 ...
 $ T5_I                        : int 64
 $ T1_Ja                       : int 256
 $ technique                   : Factor w/ 1 level "RS": 1
 $ RT1_J                       : int 8
 $ RT7_I                       : int 1
 ...
 $ T3_I                        : int 256
 $ RT1_I                       : int 8
 $ VEC6                        : Factor w/ 2 levels "False","True": 2
 $ mean_confidence_interval_inf: num 0.0755
 $ T1_I                        : int 16
 $ T1_J                        : int 16
 $ T5_Ia                       : int 2048
 $ T3_Ja                       : int 1024
 ...
 $ T7_J                        : int 16
 $ VEC3                        : Factor w/ 2 levels "False","True": 2
 $ T7_I                        : int 32
 $ VEC2                        : Factor w/ 2 levels "False","True": 1
 $ T7_Ia                       : int 256
 $ VEC8                        : Factor w/ 2 levels "False","True": 1
 $ U10_I                       : int 4
 $ U6_I                        : int 16
 $ T1_Ia                       : int 64
 ...
 $ U7_I                        : int 4
 $ U7_J                        : int 1
 $ cost_std                    : num 0.000546
 $ runs                        : int 35
 $ mean_confidence_interval_sup: num 0.0759
 $ U2_I                        : int 16
 $ cost_mean                   : num 0.0757
 $ U9_I                        : int 4
 $ RT3_I                       : int 1
 $ RT3_J                       : int 8
...
ggplot(data) +
    geom_histogram(aes(cost_mean), binwidth = 0.01) +
    geom_vline(aes(xintercept = cost_mean), O3_baseline, color = "black", linetype = 2) +
    coord_cartesian(xlim = c(0, max(data$cost_mean)), ylim = c(0, 200)) +
    geom_text(aes(x = cost_mean - 0.02, y = 200, label = "-O3 baseline"), data = O3_baseline) +
    theme_bw()
#+END_SRC

#+RESULTS:
[[file:/tmp/babel-uAeVrZ/figureP5SsUi.png]]
*** [2018-07-25 Wed]
...
ggplot(data) +
    geom_histogram(aes(cost_mean), binwidth = 0.01) +
    geom_vline(aes(xintercept = cost_mean), O3_baseline, color = "black", linetype = 2) +
    coord_cartesian(xlim = c(0, max(data$cost_mean)), ylim = c(0, 130)) +
    geom_text(aes(x = cost_mean + 0.1, y = 130, label = "-O3 baseline"), data = O3_baseline) +
    theme_bw()
#+END_SRC

#+RESULTS:
[[file:/tmp/babel-C2GnlH/figuresHW2CG.png]]
...
#+RESULTS:
:  [1] "DLMT_large" "DLMT_large" "DLMT_large" "DLMT_large" "DLMT_large"
:  [6] "DLMT_large" "DLMT_large" "DLMT_large" "DLMT_large" "DLMT_large"
: [11] "RS_large"   "RS_large"   "RS_large"   "RS_large"   "RS_large"
: [16] "RS_large"   "RS_large"   "RS_large"   "RS_large"   "RS_large"
: [21] "DLMT_small" "DLMT_small" "DLMT_small" "DLMT_small" "DLMT_small"
: [26] "DLMT_small" "DLMT_small" "DLMT_small" "DLMT_small" "DLMT_small"
: [31] "RS_small"   "RS_small"   "RS_small"   "RS_small"   "RS_small"
: [36] "RS_small"   "RS_small"   "RS_small"   "RS_small"   "RS_small"
...
#+RESULTS:
[[file:/tmp/babel-AiL4Pb/figurebmoJWF.png]]
** August
*** [2018-08-22 Wed]
...
- Additionally, initial experiments with O2, O1 and O0 showed similar sample
  distributions as in the O3 case
...
- Invite Steven
- I believe we can write a nice paper for IPDPS 19, or to
...
*** [2018-08-24 Fri]
**** Plotting Orio Experiments Data (dgemv3, O3, Xeon E5 2630 v2)
#+HEADER: :results output
#+BEGIN_SRC shell
git clone https://github.com/phrb/orio_experiments.git || (cd orio_experiments && git pull)
#+END_SRC
...
data <- read.csv("orio_experiments/testsuite/SPAPT/dgemv3/xeon_e5_2630_v2/dgemv3_O3/results.csv")
...
#+RESULTS:
[[file:/tmp/babel-AiL4Pb/figurexz5IEn.png]]
